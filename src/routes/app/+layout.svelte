<script lang="ts">
	...
	const navItems = [
		...
		{ href: '/app/collections', label: 'Collections', icon: 'collection' },
		{ href: '/app/inks', label: 'Inks', icon: 'ink' },
		{ href: '/app/recipes', label: 'Recipes', icon: 'recipe' },
		{ href: '/app/manufacturers', label: 'Makers', icon: 'manufacturer' },
...
{#if auth.loading}
	<div class="flex min-h-screen items-center justify-center bg-background">
		<div class="text-center">
			<div class="mx-auto mb-3 h-8 w-8 animate-spin rounded-full border-3 border-primary border-t-transparent"></div>
...
{:else if auth.user}
	<div class="flex min-h-screen bg-background">
		...
		{#if sidebarOpen}
			<div
				class="fixed inset-0 z-40 bg-black/50 backdrop-blur-sm animate-fade-in lg:hidden"
		...
		<!-- Sidebar -->
		<aside
			class="fixed inset-y-0 left-0 z-50 flex w-72 flex-col border-r border-border bg-card transition-transform duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] lg:static lg:translate-x-0 {sidebarOpen ? 'translate-x-0' : '-translate-x-full'}"
		>
			<!-- Logo -->
					...
					{#each navItems as item}
						<li>
							<a
								href={item.href}
								onclick={() => sidebarOpen = false}
								class="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-all duration-150 {isActive(item.href)
			...
			<div class="border-t border-border p-4">
				<div class="flex items-center gap-3">
					...
					<div class="min-w-0 flex-1">
						<p class="truncate text-sm font-medium text-foreground">{auth.user.email}</p>
					</div>
		...
		<div class="flex flex-1 flex-col">
			<!-- Top bar (mobile) -->
			<header class="sticky top-0 z-30 flex h-14 items-center gap-3 border-b border-border bg-card/80 px-4 backdrop-blur-lg lg:hidden">
				<button
				...
				<div class="flex items-center gap-2">
					<div class="flex h-6 w-6 items-center justify-center rounded-md bg-primary text-[10px] font-bold text-primary-foreground">PV</div>
					<span class="font-serif text-base font-bold text-foreground">PenVault</span>
				</div>
			</header>
